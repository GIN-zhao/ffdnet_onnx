...
def main():
    # 参数设置
    noise_level = 40  # 噪声标准差（针对 uint8 图片尺度 0~255）
    model_name = 'ffdnet_gray_clip'  # 模型名称，包含 'color' 则认为是彩色模型，否则为灰度模型
    testset_dir = "./examples"  # 测试图片所在目录
    results_dir = f"./examples_{model_name}"
    ...
    # 加载 ONNX 模型
    session = ort.InferenceSession(f"{model_name}.onnx", providers=['CUDAExecutionProvider', 'CPUExecutionProvider'],opset_version=11)
    ...
    for img_path in image_paths:
        ...
        st = time.time() * 1000
        # 预处理：读取、归一化、加噪
        input_img = preprocess_image(img_path, n_channels, 25, use_clip)
        # 构造模型输入
